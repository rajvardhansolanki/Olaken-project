...
const Chat: React.FC = () => {
    ...
    const onJoiningRoom = async () => {
        const { buyerInfo, id } = JSON.parse(localStorage.getItem('product_detail'));
                ...
                >
                    <Stack sx={{ width: "90%", }}>
                        <Typography gutterBottom variant={"body2"}>
                            {product && product?.owner?.name} - {buyerInfo && buyerInfo?.firstName + ' ' + buyerInfo?.lastName}
                        </Typography>
                        {product &&
                            <Typography gutterBottom variant={"h6"} whiteSpace='nowrap' overflow='hidden' textOverflow="ellipsis" sx={{ display: "flex", alignItems: "center" }}>
                                {product?.title}
                                <Launch sx={{ ml: "10px" }} onClick={() => router.push("/product/[slug]", `/product/${slug(product?.title)}-${product?._id}`)} />
                            </Typography>
                        }
                        {/* <Typography gutterBottom variant={"body2"}>
