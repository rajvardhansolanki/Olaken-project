...
const AddProduct: React.FC<IProduct> = ({
  ...
  const onDropVideoFile = useCallback(
    async (acceptedFiles: File[], fileRejections: FileRejection[]) => {
      const processVideoFiles = async () => {
        ...
        for (const file of acceptedFiles) {
          ...
          if (typeof duration === "number" && duration <= 58) {
                ...
                errors: [
                  {
                    ...
                    message: "Video duration exceeds 58 seconds",
                  },
                ],
              },
            ]);
  ...
  const removeFile = (name: any) => {
    const updatedFiles = files.filter((file, index) => index !== name);
    setFiles(updatedFiles);
    setValue("file", updatedFiles, { shouldValidate: true });
  };

  const removeVideoFile = (name: any) => {
    const updatedFiles = videoFiles.filter((file, index) => index !== name);
    setVideoFiles(updatedFiles);
    setValue("videoFile", updatedFiles, { shouldValidate: true });
  };

  const removeRejected = (name) => {
    setRejected((files) => files.filter((file, index) => index !== name));
  };
  ...
  return (
    <>
      ...
      >
        ...
        >
          ...
          >
            ...
            >
                              ...
                              {videoFiles.map((file, index) => {
                                return (
                                  <Grid
                                    item
                                    xs={12}
                                    sm={file.length === 1 ? 12 : 6}
                                    md={file.length === 1 ? 12 : 4}
                                    lg="auto"
                                    key={index}
                                    justifyContent="center"
                                    alignItems="center"
              ...
              {rejected.length > 0 && (
                <Typography color="red">Rejected Files</Typography>
              )}
        ...
        <Box
          sx={{
            marginBottom: isGlobal ? "0px" : "14px",
          }}
        >
          <Controller
            control={control}
