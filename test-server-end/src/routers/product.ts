...
import CineName from "../Models/Cine";
...
router.get("/product/:id", async (req: Request, res: Response) => {
  ...
  try {
    ...
    const aProducts: Iads[] = await Ads.find({
...
router.patch(
  ...
  async (req: Request, res: Response) => {
    ...
    try {
      ...
      // }
      updates.forEach((update) => ((product as any)[update] = data[update]));
      await product.save();
...
router.get("/products", async (req: Request, res: Response) => {
  try {
    const productsFind: IProduct[] = await Product.find({
      active: true,
      publish: true,
    })
      .where("quantity")
...
router.get("/products/recent", async (req: Request, res: Response) => {
  try {
    const recentProducts: IProduct[] = await Product.find({
      active: true,
      publish: true,
      videos: { $exists: true, $not: { $size: 0 } }, // Added this condition
    })
      .populate({
        path: "owner",
        model: Store,
...
router.get('/chine-next', async (req, res) => {
  try {
    const { userId, productId } = req.query;

    const cineName = await CineName.findOne({ userId, productId });

    if (cineName) {
      if (cineName.isVideoPlayed) {
        res.status(200).json({
          message: 'Video has already been played for this user and product.',
          cineName,
        });
      } else {
        cineName.isVideoPlayed = true;
        await cineName.save();

        res.status(200).json({
          message: 'Video marked as played for this user and product.',
          cineName,
        });
      }
    } else {
      const newCineName = await CineName.create({ userId, productId, isVideoPlayed: true });

      res.status(200).json({
        message: 'Document does not exist in CineName collection, saved successfully and video marked as played.',
        cineName: newCineName,
      });
    }
  } catch (error) {
    console.error(error);
...
router.get('/cineNames', async (req, res) => {
  try {
    const cineNames = await CineName.find();
    res.json(cineNames);
  } catch (error) {
...
router.put('/toggle-like-cinename', async (req, res) => {
  try {
    const { userId, productId } = req.body;

    // Check if the cineName document exists
    const cineName = await CineName.findOne({ userId, productId });

    if (cineName) {
      // Toggle the like status
      cineName.hasUserLiked = !cineName.hasUserLiked;

      // Update the likes count based on the like status
      if (cineName.hasUserLiked) {
        cineName.likes += 1;
      } else {
        cineName.likes -= 1;
      }
      ...
      res.status(200).json({
        message: `Like status toggled for this user and product. New like count: ${cineName.likes}`,
        cineName,
      });
    } else {
      // If the document does not exist, create a new one with likes set to 1 and hasUserLiked set to true
      const newCineName = await CineName.create({ userId, productId, likes: 1, hasUserLiked: true });

      res.status(200).json({
        message: 'Document does not exist in CineName collection, saved successfully, and like added.',
        cineName: newCineName,
      });
    }
  } catch (error) {
...
router.put('/share-cinename', async (req, res) => {
  try {
    const { userId, productId } = req.body;
    ...
    if (cineName) {
      ...
      cineName.shareCount += 1;
      await cineName.save();

      res.status(200).json({
        message: 'Share count incremented for this user and product.',
        cineName,
      });
    } else {
      // If the document does not exist, create a new one with share count set to 1
      const newCineName = await CineName.create({ userId, productId, shareCount: 1 });

      res.status(200).json({
        message: 'Document does not exist in CineName collection, saved successfully, and share count incremented.',
        cineName: newCineName,
      });
    }
  } catch (error) {
